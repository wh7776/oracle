...
<img src="表和索引创建.PNG" alt="表和索引创建图" style="zoom:60%;" />
...
with T as
(SELECT * FROM orders A join (select order_id as oid1,sum(product_num*product_price) as 折扣前合计 from order_details group by order_id) B ON A.order_id=B.oid1 )
SELECT T.*,折扣前合计-discount as 应收货款  FROM T;
